...
# HTTP API
## Add monitoring
`curl -X POST http://localhost:80/monitorings/ -H 'Content-Type: text/plain' -d tcp://www.google.de:12345`

## Delete monitoring
`curl -X DELETE http://localhost:80/monitorings/2`

## Get states as plaintext
`curl -X GET http://localhost:80/monitorings/ -H 'Accept: text/plain'`
```
...
## Get states as JSON
`curl -X GET http://localhost:80/monitorings/ -H 'Accept: application/json'`
```
[
    {
        "serviceState": "Up",
